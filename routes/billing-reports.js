...
router.get("/api/reports/billing/summary", async (req, res) => {
  try {
    ...
    const totalBilled = billedResult.rows[0].total_billed;
...
router.get("/api/reports/billing/ageing", async (req, res) => {
  try {
    ...
    res.json({
      d0_30: result.rows[0].d_0_30 || 0,
      d31_60: result.rows[0].d_31_60 || 0,
      d61_90: result.rows[0].d_61_90 || 0,
      d90_plus: result.rows[0].d_90_plus || 0
    });

  } catch (err) {
    console.error("Invoice ageing error:", err.message);
    res.status(500).json({
      d0_30: 0,
      d31_60: 0,
      d61_90: 0,
